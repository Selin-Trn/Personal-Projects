...
b. 1.5 lt plastic bottle with large lid (to allow the flow of kibbles and the motion of the mixer)
c. 1 lt platic bottle (for the outside of the container, otherwise the kibbles scatter everywhere)
...
i. 1602A 16x2 LCD with I2C/IIC chip (this chip reduces the amount of cables to connect to the Arduino from 12 to 4)
j. A breadboard (I also used a mini breadboard to seperate the LEDs and the button, but its not essential)
k. Push button (to pour down a portion ff kibbles and reset the timer)
...
Hotglue gun (to glue the LCD, the servos and the bottles to the container -also the mixer to the 360 servo)
...
LEGO Mixer:
The reason I added a mixer component: After a while the kibbles startes clumping at the mouth of the bottle and stopped pouring down.
You can try making this component by getting creative, you don't really need LEGO parts.

!!! Placing the mixer inside the bottle is a HASSLE. After gluing the bottle on the container you might not be able to change the pieces. Plan before putting everything together and make sure everything fits, do not make the mistakes I did :')
...
Serving Size:
Increase/decrease the amount of iterations inside turnServo() function to adjust serving size.
